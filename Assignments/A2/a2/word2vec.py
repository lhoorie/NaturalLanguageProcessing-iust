...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    p = softmax(np.dot(outsideVectors, centerWordVec))
    loss = - np.log(p[outsideWordIdx])
    diff = p.copy() 
    diff[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, diff)
    gradOutsideVecs = np.dot(np.expand_dims(diff, axis=-1) , np.expand_dims(centerWordVec, axis=0))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    v_c = centerWordVec
    loss = -np.log(sigmoid(np.dot(u_o, v_c))) - np.sum(np.log(sigmoid(np.dot(-u_k, v_c))))

    gradCenterVec = (sigmoid(np.dot(u_o, v_c)) - 1) * u_o + np.matmul(1 - sigmoid(np.dot(-u_k, v_c)), u_k)

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(np.dot(u_o, v_c)) - 1) * v_c
    for i, n in enumerate(negSampleWordIndices):
        gradOutsideVecs[n] += (1 - sigmoid(np.dot(-u_k, v_c))[i]) * v_c
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        l, dv_c, dU = word2vecLossAndGradient(centerWordVectors[centerWordIdx], word2Ind[w], outsideVectors, dataset)
        loss += l
        gradCenterVecs[centerWordIdx] += dv_c
