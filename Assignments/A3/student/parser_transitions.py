...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i, s  in enumerate(minibatch):
            s.parse_step(transitions[i])
            if  len(s.stack) == 1 and len(s.buffer) == 0:
                unfinished_parses.remove(s)
    dependencies = [record.dependencies for record in partial_parses]
    return dependencies
